...
from .models import (
    Platforms, Categories, SecurePass, SecureItemInfo, ItemSecretIngredient, ItemSecret
)
from .forms import (
    SecurePassForm, UpdateSecurePasscode, AccounFormForPasscode, SecureItemInfoForm_1, SecureItemInfoForm_2, InfoSecretForm, InfoIngredientForm
)
from toolkit import PasscodeSecurity, InformationSecurity, MixinTrick as MTCK
...
# maybe decorator
# def checkUserPascodeSet(request):
#   """checking if the user has setup his/her secure passcode"""
...
def home(request):
    ...
    if request.user.is_authenticated:
        user_items = SecureItemInfo.objects.filter(i_owner=request.user.securepass).order_by('-date_created')[:10]
        context = {
...
def searchTrustedUser(request, item_id):
  ...
  try:
    ...
    # This is a filter that filter user matching a giving query.
    search_result = User.objects.filter(Q(first_name=search_panel) | Q(last_name=search_panel) | Q(username=search_panel) | Q(phone_number=search_panel) | Q(email=search_panel)).order_by('-date_joined')
...
def addTrustedUser(request, item_id, user_id):
    ...
    flash_msg.success(request, f'You added {u_t.first_name} as trusted user in your item ({i_id.custom_platform})')
    return redirect(reverse('secureapp:item_info', kwargs={'item_id': item_id}))
...
def removeTrustedUser(request, item_id, user_id):
    ...
    flash_msg.warning(request, f'You remove {u_t.first_name} as trusted user in your item ({i_id.custom_platform})')
    return redirect(reverse('secureapp:item_info', kwargs={'item_id': item_id}))
...
def itemInfo(request, item_id):
    ...
    item_unveil_secret = ItemSecret.objects.get(rely_on_id=item_unveil_info.id) # secret
    item_unveil_ingre = ItemSecretIngredient.objects.get(rely_on_id=item_unveil_info.id) # ingredient
    ...
    i_card_valid_range_pub = item_unveil_info.i_card_valid_range # card_valid_range public key
    ...
    my_result = MTCK._3_to_3(item_key, item_the_hash, item_the_private, item_ingredient)
    # OUTPUT:
    ...
    if i_username_pub != '' and i_username_pub != None:
        decrypt_i_username = _BASE_.decrypt_info(i_username_pub.encode('utf-8'), my_result[3])
    else:
        decrypt_i_username = ''
    # for phone
    if i_phone_pub != '' and i_phone_pub != None:
        decrypt_i_phone = _BASE_.decrypt_info(i_phone_pub.encode('utf-8'), my_result[3])
    else:
        decrypt_i_phone = ''
    # for email
    if i_email_pub != '' and i_email_pub != None:
        decrypt_i_email = _BASE_.decrypt_info(i_email_pub.encode('utf-8'), my_result[3])
    else:
        decrypt_i_email = ''
    # for password
    if i_password_pub != '' and i_password_pub != None:
        decrypt_i_password = _BASE_.decrypt_info(i_password_pub.encode('utf-8'), my_result[3])
    else:
        decrypt_i_password = ''
    # for passphrase
    if i_passphrase_pub != '' and i_passphrase_pub != None:
        decrypt_i_passphrase = _BASE_.decrypt_info(i_passphrase_pub.encode('utf-8'), my_result[3])
    else:
        decrypt_i_passphrase = ''
    # for api
    if i_api_pub != '' and i_api_pub != None:
        decrypt_i_api = _BASE_.decrypt_info(i_api_pub.encode('utf-8'), my_result[3])
    else:
        decrypt_i_api = ''
    # for ssh_key_pub
    if i_ssh_key_pub_pub != '' and i_ssh_key_pub_pub != None:
        decrypt_i_ssh_key_pub = _BASE_.decrypt_info(i_ssh_key_pub_pub.encode('utf-8'), my_result[3])
    else:
        decrypt_i_ssh_key_pub = ''
    # for ssh_key_prt
    if i_ssh_key_prt_pub != '' and i_ssh_key_prt_pub != None:
        decrypt_i_ssh_key_prt = _BASE_.decrypt_info(i_ssh_key_prt_pub.encode('utf-8'), my_result[3])
    else:
        decrypt_i_ssh_key_prt = ''
    # for card_no
    if i_card_no_pub != '' and i_card_no_pub != None:
        decrypt_i_card_no = _BASE_.decrypt_info(i_card_no_pub.encode('utf-8'), my_result[3])
    else:
        decrypt_i_card_no = ''
    # for card_valid_range
    if i_card_valid_range_pub != '' and i_card_valid_range_pub != None:
        decrypt_i_card_valid_range = _BASE_.decrypt_info(i_card_valid_range_pub.encode('utf-8'), my_result[3])
    else:
        decrypt_i_card_valid_range = ''
    # for card_ccv
    if i_card_ccv_pub != '' and i_card_ccv_pub != None:
        decrypt_i_card_ccv = _BASE_.decrypt_info(i_card_ccv_pub.encode('utf-8'), my_result[3])
    else:
        decrypt_i_card_ccv = ''
    # for card_pin
    if i_card_pin_pub != '' and i_card_pin_pub != None:
        decrypt_i_card_pin = _BASE_.decrypt_info(i_card_pin_pub.encode('utf-8'), my_result[3])
    else:
        decrypt_i_card_pin = ''
...
def setSecurePasscode(request):
  ...
  if request.method == 'POST':
    ...
    """
    acc_form = AccounFormForPasscode(request.POST, instance=request.user)
    pass_form = SecurePassForm(request.POST, instance=request.user.securepass)
    ...
    if pass_form.is_valid() and acc_form.is_valid():
      ...
      if verify_user_email != user_email:
        flash_msg.warning(request, f'Email address you enter do not match your account emaill address')
        pass_form = SecurePassForm(instance=request.user.securepass)
      ...
      auth_result = auth_pass.get_hash(salt_q, iter_q, verify_new_passcode_again) # the key of the passcode
      hashed_passcode = auth_result[0] # second hash after been encoded using base64
      pass_ingredient = auth_result[1] # the ingredient of the hash (salt + iteration + second_hash) respectively
...
@login_required
def updateSecurePasscode(request):
  ...
  old_ingredient = request.user.securepass.passcode_ingredient
  ...
  """
  slicing ingredient from old_ingredient, starting from index zero
  to the actual length of the (real_hash) above
  """
  ...
  if request.method == 'POST':
    """
    The acc_form is not going to be use in HTML template,
    it will be use only within this function, so that to renew (second hash i.e passcode_hash)
    in the custome user model (UserAccount)
    """
    acc_form = AccounFormForPasscode(request.POST, instance=request.user)
    pass_form = UpdateSecurePasscode(request.POST, instance=request.user.securepass)
    ...
    if pass_form.is_valid() and acc_form.is_valid():
      ...
      if verify_user_email != user_email:
        flash_msg.warning(request, f'Email address you enter do not match your account emaill address')
        pass_form = UpdateSecurePasscode(instance=request.user.securepass)
      ...
      """
        comparing the second hash (old), with the new second hash (renew), like wise also;
        comparing the ingredient (old), with the new ingredient (renew),
        both most match to what we assign to them, in other to pass to the next step
      """
      ...
      """
      comparing the two html fields with each other, and
      comparing the old ingredient with the confirm one, for the renewal
      """
      if verify_new_passcode == new_raw_passcode and slice_ingre == confirm_ingredient:
        ...
        pass_result = renew_pass.get_hash(new_salt, new_iter, new_raw_passcode) # the key of the passcode
        new_passcode = pass_result[0] # second hash after been encode using base64
        new_ingredient = pass_result[1] # the ingredient of the hash (salt + iteration + second_hash) respectively
...
def newItem(request):
  ...
  site_platform = Platforms.objects.all()
  site_categories = Categories.objects.all()
  """
    ...
    so we slice the third index `i[3]` which is the platform_value
  """
  l_platform = [i[3] for i in Platforms.objects.values_list()]
  ...
  """
  l_category = [i[3] for i in Categories.objects.values_list()]
  ...
  if request.method == 'POST':
    ...
    'site_categories': site_categories,
    'site_platform': site_platform,}
        flash_msg.warning(request, f'May be you edited your visibility field(s), thats why we redirect you back')
    ...
    'site_categories': site_categories,
    'site_platform': site_platform,}
        flash_msg.warning(request, f'May be you edited your category field(s), thats why we redirect you back')
        return render(request, 'secureapp/new_item_1.html', context)
    ...
    'site_categories': site_categories,
    'site_platform': site_platform,}
        flash_msg.warning(request, f'May be you edited your platform field(s), thats why we redirect you back')
        return render(request, 'secureapp/new_item_1.html', context)
...
def newItemFields(request):
  the_i_owner = SecurePass.objects.get(id=request.user.securepass.id)
  ...
  if request.method == 'POST':
    ...
    secret_form = InfoSecretForm(request.POST)
    ingredient_form = InfoIngredientForm(request.POST)
    
    if form.is_valid() and secret_form.is_valid() and ingredient_form.is_valid():
      """Items values from html template"""
      ...
      # our default category list
      l_category = ['social media', 'cloud', 'credit card', 'other']
      if i_category not in l_category:
        context = {'form': form}
        flash_msg.warning(request, f'May be you edited your category field(s), thats why we redirect you back')
        return render(request, 'secureapp/new_item_1.html', context)
      
      # our default platform list
      l_platform = ['aws', 'heroku', 'linode', 'facebook', 'twitter', 'instagram', 'linkedin', 'id card', 'store id', 'credit card', 'visa card', 'github', 'custom']
      ...
      # checking to see if the fields all are empty
      if i_username == '' and i_phone == None and i_email == None and i_password == None and i_passphrase == None and i_api == None and i_ssh_key_pub == None and i_ssh_key_prt == None and i_card_no == None and i_card_valid_range == None and i_card_ccv == None and i_card_pin == None:
        ...
        context = {'form': form}
        flash_msg.warning(request, f'All fields can\'t be empty, try again')
        return redirect('secureapp:new_item')
      ...
      ITEM_salt_generation = _BASE_.the_key() # randomly generated salt for pbkdf2_hmac hashing
      ...
      ITEM_get_hash_info = _BASE_.get_hash(ITEM_salt_generation, ITEM_get_iteration, ITEM_private)
      ITEM_hash = ITEM_get_hash_info[0] # second hash
      ITEM_ingredient = ITEM_get_hash_info[1] # (salt, itter, hash_result)
      ITEM_salt__itter = ITEM_ingredient[:-len(ITEM_hash)]
      ITEM_salt = ITEM_salt__itter[:-3]
      ITEM_iteration = ITEM_salt__itter[len(ITEM_salt):]
      INS_result = MTCK._instance_item_data(ITEM_iteration, ITEM_hash, ITEM_salt, ITEM_private)
      # OUTPUT:
      ...
      # the model in which secret and ingredient tables rely on
      instance_to_rely = SecureItemInfo.objects.get(id=instance.id)
      
      # for secret
      instance_secret = secret_form.save(commit=False)
      instance_secret.rely_on_id = instance_to_rely.id
      instance_secret.the_hash = INS_result[1]
      instance_secret.the_private = INS_result[2]
      instance_secret.save()
      
      # for ingredient
      instance_ingedient = ingredient_form.save(commit=False)
      instance_ingedient.rely_on_id = instance_to_rely.id
      instance_ingedient.ingredient = INS_result[3]
