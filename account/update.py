...
def update_passcode(request):
    ...
    if request.method == 'POST':
        ...
        pass_form = PassCodeForm_1x(request.POST, instance=request.user.passcode)
        
        # Grabbing data from html template
        verify_user_email = request.POST['verify_email']
        old_raw_passcode = request.POST['old_passcode']
        # old_raw_passcode = request.POST['old_passcode']
        new_raw_passcode = request.POST['passcode_ingredient']
        verify_new_passcode = request.POST['verify_passcode']
        ...
        if verify_user_email != user_email:
            ...
            pass_form = PassCodeForm_1x(instance=request.user.passcode)
